...
object RegolithReservoir2 {
  def main(args: Array[String]): Unit = {
    val str = Source.fromFile(System.getProperty("user.dir") + "/2022/day14/input.txt")
    val input = str.getLines().toSeq

    val (tunnel, bottom) = buildTunnel(input, Array.fill(700, 190)("."), 0)
    (0 until 700).foreach(i => tunnel(i)(bottom + 2) = "#")
    runSand(tunnel, bottom + 2)
  }
  ...
  def runSand(tunnel: Array[Array[String]], bottom: Int): Unit = {
    def run(x: Int, startCount: Int, isCheck: Boolean): Boolean = {
      ...
      while (true) {
        ...
        else if (count > bottom) return false
        if (tunnel(x)(count).equals(".")) count += 1
        else {
          val canRight = if (tunnel(x - 1)(count) == ".") run(x - 1, count, isCheck = true) else false
          if (canRight && isCheck) return true
          if (canRight) count = 0
          else {
            val canLeft = if (tunnel(x + 1)(count) == ".") run(x + 1, count, isCheck = true) else false
