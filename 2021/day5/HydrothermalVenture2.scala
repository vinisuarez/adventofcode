...
object HydrothermalVenture2 {
  ...
  private def markTheBoard(board: Array[Array[Int]], direction: Direction): Array[Array[Int]] = {
    ...
    else if (direction.init.x != direction.end.x && direction.init.y != direction.end.y) markDiagonal(board, direction)
    else {
  ...
  private def markDiagonal(board: Array[Array[Int]], direction: Direction): Array[Array[Int]] = {
    if (direction.init.x > direction.end.x) {
      if (direction.init.y > direction.end.y) {
        markTheBoard(board, Direction(Coordinate(direction.init.x - 1, direction.init.y - 1), direction.end))
      } else {
        markTheBoard(board, Direction(Coordinate(direction.init.x - 1, direction.init.y + 1), direction.end))
      }
    } else {
